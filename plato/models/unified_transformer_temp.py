...
class UnifiedTransformer(ModelBase):
    ...
    def __init__(self, name_scope, hparams, generator, dtype="float32"):
        ...
        self.lada = layers.create_parameter(name='lambda_point',shape=[2],default_initializer=fluid.initializer.NumpyArrayInitializer(np.array([0.5,0.5])),dtype="float32")
    ...
    def _discriminator_network(self, input_mask, embed, batch_size, src_len, tgt_len, pos_embed):
        ...
        if batch_size > 1:
            ...
            neg_tgt_embed = tgt_embed #这块可以修改，妈的，你他妈的随机sample呢？
    ...
    def _generation_network(self, input_mask, embed, batch_size, src_len, tgt_len, latent_embed,knw_len,src_type,src_token):
        ...
        if self.use_pointer_network:
            ...
            #dec_probs = layers.elementwise_add(x=self.lada*dec_probs,y=(1-self.lada)*pointer_probs)
            dec_probs = layers.elementwise_add(x=dec_probs,y=pointer_probs)

...
def repeat(var, times):
    if isinstance(var, list):
        ...
        var = F.unsqueeze(var, [1]) ## 增加第一个维度
        expand_times = [1] * len(var.shape)
        expand_times[1] = times #[1,times,1]，如果shape是3维
        dtype = var.dtype
        var = layers.cast(var, "float32")
        var = layers.expand(var, expand_times) #对times维度进行填充
        shape = [var.shape[0] * var.shape[1]] + var.shape[2:] #维度0和维度1进行合并
...
def gather(var, idx):#拿到对应index的元素,index是一个tensor
    if isinstance(var, list):
...
class Sampling(Generator):
    ...
    def __call__(self, step_fn, state):
        ...
        for step in range(1, self.max_gen_len + 1):
            pre_ids = predictions[:, -1:] #（batch_size,1）
            state["pred_token"] = F.unsqueeze(pre_ids, [2]) ##（batch_size,1,1）
            if step > 1:
                state["pred_mask"] = 1 - F.equal(state["pred_token"], self.pad_id) # 如果pad_id，则为0
                state["pred_pos"] = state["pred_pos"] + 1 #pos可能是一个常量？
            scores, state = step_fn(state) #decoding one step
            ...
            #整个意思是，如果前一个为pad或eos然后面的都是eos或者pad。
            # [batch_size,vocab_size]*[batch_size,1],下面第一个值前一个位置为pad,eos位置的vocab-size的维度全为0，其它不变
            scores = scores * (1 - pre_eos_mask) + layers.expand(pre_eos_mask, [1, vocab_size]) * scores_after_end #score_after_end除了 pad位置为0 ，其它的都为-1e10，size为【vocab_size】

            scores = scores / self.temperature
            preds = self._sampling(scores)

            predictions = layers.concat([predictions, F.unsqueeze(preds, [1])], axis=1)#[batch_size,1]
...
class TopPSampling(Sampling):
    ...
    def _sampling(self, logits):
        ...
        for p in probs.numpy():
            ids = np.argsort(-p) #对下标进行排序
            p = p[ids] #按照从大到小选择
            c_p = np.cumsum(p)  #累加概率
